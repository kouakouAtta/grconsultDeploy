...
$(document).ready(function(){$("#mvtagentpro-datatable").DataTable(
{
    iDisplayLength: 10,//liste Des Mvt Agent ProposerByAdminAction
    sAjaxSource: basePath + '/ajaxservices/liste-des-mvt-agent-proposer-by-admin',
    sServerMethod: 'post',
...
function getBtn(id, isvalide){
    ...
    if(isvalide!=0)
    {
     ...
     btnDelete = 
            ...
            "  </a> ";
     btnDelete += 
            "        <a href='"+basePath+"/mouvement-agent/modifier/"+id+"' data-toggle='tooltip' data-placement='top' title='Valider' " +
            "            class='btn btn-outline-success btn-rounded waves-effect waves-light'> " +
            "             <i class='feather-edit'></i> " +
            "        </a> ";
    
     btnDelete += 
            "        <a href='"+basePath+"/mouvement-agent/supprimer/"+id+"' data-toggle='tooltip' data-placement='top' title='Refuser' " +
