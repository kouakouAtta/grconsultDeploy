...
$(document).ready(function(){$("#pc-datatable").DataTable(
{
    iDisplayLength: 10,
    sAjaxSource: basePath + '/ajax/liste-des-pcs',
    sServerMethod: 'post',
    ...
    fnRowCallback: function(nRow, aData, iDisplayIndex){
        ...
        $('td:eq(4)', nRow).html(aData[7]+" Go");
        $('td:eq(5)', nRow).html(aData[8]+" Go");
        $('td:eq(6)', nRow).html(aData[9]+" Hertz");
        $('td:eq(7)', nRow).html(aData[10]+" \"");// à revoir
        $('td:eq(8)', nRow).html(getBtn(aData[0]));
    }
});});
