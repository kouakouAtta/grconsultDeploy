...
$(document).ready(function(){$("#liste-mes-demandes-datatable").DataTable(
{
    iDisplayLength: 10,
    sAjaxSource: basePath + '/ajax/mes-demandes',
    sServerMethod: 'post',
    ...
    fnRowCallback: function(nRow, aData, iDisplayIndex){
        $('td:eq(0)', nRow).html(alignCenter(aData[19]));
        $('td:eq(1)', nRow).html(alignCenter(formatDateMysql(aData[1])));
        $('td:eq(2)', nRow).html(alignCenter(getBadge(aData)));
...
function getBtn(aData){
    ...
    var btnDelete = 
            "        <a href='"+basePath+"/demandes/ma-demande/"+aData[0]+"'  title='Voir les détails' " +
            "            class='btn btn-outline-info btn-rounded waves-effect waves-light detailBtn'> " +
            "             <i class='feather-eye'></i> " +
            "        </a> ";
    
    if(aData[14]!=null && aData[17]==null){
        btnDelete += 
                "        <a href='"+basePath+"/demandes/attribuer-le-materiel/"+aData[0]+"' data-toggle='tooltip' data-placement='top' title='Attribuer des numéros de carton' " +
